...
#define PRTE_timex_DYN_MAX_SIZE 256

/*
 * API functions
 */
static int init(void);
static int prte_ras_timex_allocate(prte_job_t *jdata, pmix_list_t *nodes);
static void deallocate(prte_job_t *jdata, prte_app_context_t *app);
static int prte_ras_timex_finalize(void);

/*
 * RAS timex module
 */
prte_ras_base_module_t prte_ras_timex_module = {init, prte_ras_timex_allocate, deallocate,
                                                prte_ras_timex_finalize};

/* Local functions */
static int prte_ras_timex_discover(char *regexp, char *tasks_per_node, pmix_list_t *nodelist);
static int prte_ras_timex_parse_ranges(char *base, char *ranges, char ***nodelist);
static int prte_ras_timex_parse_range(char *base, char *range, char ***nodelist);
...
typedef struct {
    pmix_object_t super;
    int sjob;
} local_apptracker_timex_t;
PMIX_CLASS_INSTANCE(local_apptracker_timex_t, pmix_object_t, NULL, NULL);
...
typedef struct {
    ...
    int napps;
} local_jobtracker_timex_t;
static void jtrk_cons(local_jobtracker_timex_t *ptr)
{
    ptr->cmd = NULL;
...
static void jtrk_des(local_jobtracker_timex_t *ptr)
{
    int i;
    local_apptracker_timex_t *ap;

    if (NULL != ptr->cmd) {
        free(ptr->cmd);
    }
    for (i = 0; i < ptr->apps.size; i++) {
        if (NULL != (ap = (local_apptracker_timex_t *) pmix_pointer_array_get_item(&ptr->apps, i))) {
            PMIX_RELEASE(ap);
        }
    }
    PMIX_DESTRUCT(&ptr->apps);
}
PMIX_CLASS_INSTANCE(local_jobtracker_timex_t, pmix_list_item_t, jtrk_cons, jtrk_des);
...
static int init(void)
{
    char *timex_host = NULL;
    uint16_t port = 0;
    struct sockaddr_in address;
    int flags;
    struct hostent *h;

    if (prte_ras_timex_component.dyn_alloc_enabled) {
        if (NULL == prte_ras_timex_component.config_file) {
            pmix_show_help("help-ras-timex.txt", "dyn-alloc-no-config", true);
            return PRTE_ERR_SILENT;
        }
        /* setup the socket */
        if (PRTE_SUCCESS != read_ip_port(prte_ras_timex_component.config_file, &timex_host, &port)
            || NULL == timex_host || 0 == port) {
            if (NULL != timex_host) {
                free(timex_host);
            }
            return PRTE_ERR_SILENT;
        ...
        PRTE_OUTPUT_VERBOSE((2, prte_ras_base_framework.framework_output,
                             "ras:timex got [ ip = %s, port = %u ] from %s\n", timex_host, port,
        ...
        /* connect to the timex dynamic allocation port */
        bzero(&address, sizeof(address));
        address.sin_family = AF_INET;
        if (!pmix_net_isaddr(timex_host)) {
            /* if the ControlMachine was not specified as an IP address,
             * we need to resolve it here
             */
            if (NULL == (h = gethostbyname(timex_host))) {
                /* could not resolve it */
                pmix_show_help("help-ras-timex.txt", "host-not-resolved", true, timex_host);
                free(timex_host);
                return PRTE_ERR_SILENT;
            }
            free(timex_host);
            timex_host = strdup(inet_ntoa(*(struct in_addr *) h->h_addr_list[0]));
        }
        address.sin_addr.s_addr = inet_addr(timex_host);
        address.sin_port = htons(port);
        if (connect(socket_fd, (struct sockaddr *) &address, sizeof(address)) < 0) {
            pmix_show_help("help-ras-timex.txt", "connection-failed", true, timex_host, (int) port);
            free(timex_host);
            return PRTE_ERR_SILENT;
        }
        free(timex_host);
        ...
        if ((flags = fcntl(socket_fd, F_GETFL, 0)) < 0) {
            prte_output(0, "ras:timex:dyn: fcntl(F_GETFL) failed: %s (%d)",
                        strerror(prte_socket_errno), prte_socket_errno);
            return PRTE_ERROR;
        } else {
            flags |= O_NONBLOCK;
            if (fcntl(socket_fd, F_SETFL, flags) < 0) {
                prte_output(0, "ras:timex:dyn: fcntl(F_SETFL) failed: %s (%d)",
                            strerror(prte_socket_errno), prte_socket_errno);
                return PRTE_ERROR;
            }
        }
...
static int prte_ras_timex_allocate(prte_job_t *jdata, pmix_list_t *nodes)
{
    ...
    char *timex_node_str, *regexp;
    char *tasks_per_node, *node_tasks;
    char *tmp;
    char *timex_jobid;

    if (NULL == (timex_jobid = getenv("timex_JOBID"))) {
        /* we are not in a timex allocation - see if dyn alloc
         * is enabled
         */
        if (!prte_ras_timex_component.dyn_alloc_enabled) {
            /* nope - nothing we can do */
            prte_output_verbose(2, prte_ras_base_framework.framework_output,
                                "%s ras:timex: no prior allocation and dynamic alloc disabled",
         ...
         */
        prte_job_ident = strdup(timex_jobid);
    }

    timex_node_str = getenv("timex_NODELIST");
    if (NULL == timex_node_str) {
        ...
        pmix_show_help("help-ras-timex.txt", "timex-env-var-not-found", 1, "timex_NODELIST");
        return PRTE_ERR_NOT_FOUND;
    }
    regexp = strdup(timex_node_str);
    if (NULL == regexp) {
        PRTE_ERROR_LOG(PRTE_ERR_OUT_OF_RESOURCE);
        return PRTE_ERR_OUT_OF_RESOURCE;
    }

    if (prte_ras_timex_component.use_all) {
        /* this is an oddball case required for debug situations where
         * a tool is started that will then call mpirun. In this case,
         * timex will assign only 1 tasks/per node to the tool, but
        ...
        tasks_per_node = getenv("timex_JOB_CPUS_PER_NODE");
        if (NULL == tasks_per_node) {
            /* couldn't find any version - abort */
            pmix_show_help("help-ras-timex.txt", "timex-env-var-not-found", 1,
                           "timex_JOB_CPUS_PER_NODE");
            free(regexp);
            return PRTE_ERR_NOT_FOUND;
        }
        ...
        tasks_per_node = getenv("timex_TASKS_PER_NODE");
        if (NULL == tasks_per_node) {
            /* couldn't find any version - abort */
            pmix_show_help("help-ras-timex.txt", "timex-env-var-not-found", 1,
                           "timex_TASKS_PER_NODE");
            free(regexp);
            return PRTE_ERR_NOT_FOUND;
        }
        node_tasks = strdup(tasks_per_node);
        ...
        /* get the number of CPUs per task that the user provided to timex */
        tmp = getenv("timex_CPUS_PER_TASK");
        if (NULL != tmp) {
            cpus_per_task = atoi(tmp);
            if (0 >= cpus_per_task) {
                prte_output(0,
                            "ras:timex:allocate: Got bad value from timex_CPUS_PER_TASK. "
                            "Variable was: %s\n",
                            tmp);
    ...
    ret = prte_ras_timex_discover(regexp, node_tasks, nodes);
    free(regexp);
    free(node_tasks);
    if (PRTE_SUCCESS != ret) {
        PRTE_OUTPUT_VERBOSE((1, prte_ras_base_framework.framework_output,
                             "%s ras:timex:allocate: discover failed!",
                             PRTE_NAME_PRINT(PRTE_PROC_MY_NAME)));
        return ret;
    }
    ...
    PRTE_OUTPUT_VERBOSE((1, prte_ras_base_framework.framework_output,
                         "%s ras:timex:allocate: success", PRTE_NAME_PRINT(PRTE_PROC_MY_NAME)));
    return PRTE_SUCCESS;
}
 ...
 *
 * In order to fully support timex, we need to be able to handle
 * node regexp/task_per_node strings such as:
 * foo,bar    5,3
 * foo        5
 * foo[2-10,12,99-105],bar,foobar[3-11] 2(x10),5,100(x16)
 *
 * @param *regexp A node regular expression from timex (i.e. timex_NODELIST)
 * @param *tasks_per_node A tasks per node expression from timex
 *                        (i.e. timex_TASKS_PER_NODE)
...
static int prte_ras_timex_discover(char *regexp, char *tasks_per_node, pmix_list_t *nodelist)
{
    ...
    PRTE_OUTPUT_VERBOSE((1, prte_ras_base_framework.framework_output,
                         "%s ras:timex:allocate:discover: checking nodelist: %s",
    ...
    do {
        ...
        if (i == 0) {
            ...
            pmix_show_help("help-ras-timex.txt", "timex-env-var-bad-value", 1, regexp,
                           tasks_per_node, "timex_NODELIST");
        ...
        if (found_range) {
            ...
            ret = prte_ras_timex_parse_ranges(base, base + i + 1, &names);
            if (PRTE_SUCCESS != ret) {
                pmix_show_help("help-ras-timex.txt", "timex-env-var-bad-value", 1, regexp,
                               tasks_per_node, "timex_NODELIST");
                PRTE_ERROR_LOG(ret);
            ...
            PRTE_OUTPUT_VERBOSE((1, prte_ras_base_framework.framework_output,
                                 "%s ras:timex:allocate:discover: found node %s",
    ...
    while (begptr) {
         ...
         * descriptions than nodes. Turns out that this valid, and timex will
         * return such a thing. For instance, if I did:
         * srun -A -N 30 -w odin001
         * I would get timex_NODELIST=odin001 timex_TASKS_PER_NODE=4(x30)
         ...
         * more of a timex issue than a prte issue, since timex is OK with it,
         * I'm ok with it
         */
        ...
        if (*endptr == ',') {
            ...
            pmix_show_help("help-ras-timex.txt", "timex-env-var-bad-value", 1, regexp,
                           tasks_per_node, "timex_TASKS_PER_NODE");
            PRTE_ERROR_LOG(PRTE_ERR_BAD_PARAM);
            free(slots);
            free(orig);
            return PRTE_ERR_BAD_PARAM;
    ...
    for (i = 0; NULL != names && NULL != names[i]; ++i) {
        ...
        PRTE_OUTPUT_VERBOSE((1, prte_ras_base_framework.framework_output,
                             "%s ras:timex:allocate:discover: adding node %s (%d slot%s)",
 ...
 */
static int prte_ras_timex_parse_ranges(char *base, char *ranges, char ***names)
{
    int i, len, ret;
    char *start, *orig;
    ...
    for (orig = start = ranges, i = 0; i < len; ++i) {
        if (',' == ranges[i]) {
            ranges[i] = '\0';
            ret = prte_ras_timex_parse_range(base, start, names);
            if (PRTE_SUCCESS != ret) {
                PRTE_ERROR_LOG(ret);
                return ret;
            }
    ...
    if (start < orig + len) {
        ...
        PRTE_OUTPUT_VERBOSE((1, prte_ras_base_framework.framework_output,
                             "%s ras:timex:allocate:discover: parse range %s (2)",
 ...
 * @param ***names An argv array to add the newly discovered nodes to
 */
static int prte_ras_timex_parse_range(char *base, char *range, char ***names)
{
...
static void timeout(int fd, short args, void *cbdata)
{
    local_jobtracker_timex_t *jtrk = (local_jobtracker_timex_t *) cbdata;
    prte_job_t *jdata;

    pmix_show_help("help-ras-timex.txt", "timex-dyn-alloc-timeout", true);
    prte_output_verbose(2, prte_ras_base_framework.framework_output,
...
static void recv_data(int fd, short args, void *cbdata)
{
    ...
    local_jobtracker_timex_t *ptr, *jtrk;
    local_apptracker_timex_t *aptrk;
    ...
    prte_output_verbose(2, prte_ras_base_framework.framework_output,
                        "%s ras:timex: dynamic allocation - data recvd",
                        PRTE_NAME_PRINT(PRTE_PROC_MY_NAME));
    ...
    prte_output_verbose(2, prte_ras_base_framework.framework_output,
                        "%s ras:timex: dynamic allocation msg: %s",
    ...
    if (0 == nbytes || 0 == strlen(recv_msg) || strstr(recv_msg, "failure") != NULL) {
         ...
         * message
         */
        pmix_show_help("help-ras-timex.txt", "timex-dyn-alloc-failed", true,
    ...
    for (item = pmix_list_get_first(&jobs); item != pmix_list_get_end(&jobs);
        ...
        ptr = (local_jobtracker_timex_t *) item;
    ...
    if (NULL == jtrk) {
        pmix_show_help("help-ras-timex.txt", "timex-dyn-alloc-failed", true, "NO JOB TRACKER");
        PRTE_ACTIVATE_JOB_STATE(NULL, PRTE_JOB_STATE_ALLOC_FAILED);
    ...
    for (i = 1; NULL != alloc[i]; i++) {
        if (PRTE_SUCCESS != parse_alloc_msg(alloc[i], &idx, &sjob, &nodelist, &tpn)) {
            pmix_show_help("help-ras-timex.txt", "timex-dyn-alloc-failed", true, jtrk->cmd);
            PRTE_ACTIVATE_JOB_STATE(jdata, PRTE_JOB_STATE_ALLOC_FAILED);
            pmix_argv_free(alloc);
            if (NULL != nodelist) {
                free(nodelist);
        ...
        /* track the timex jobid */
        if (NULL
            == (aptrk = (local_apptracker_timex_t *) pmix_pointer_array_get_item(&jtrk->apps, idx))) {
            aptrk = PMIX_NEW(local_apptracker_timex_t);
            pmix_pointer_array_set_item(&jtrk->apps, idx, aptrk);
        }
        aptrk->sjob = sjob;
        /* since the nodelist/tpn may contain regular expressions, parse them */
        if (PRTE_SUCCESS != (rc = prte_ras_timex_discover(nodelist, tpn, &ndtmp))) {
            PRTE_ERROR_LOG(rc);
            PRTE_ACTIVATE_JOB_STATE(jdata, PRTE_JOB_STATE_ALLOC_FAILED);
            pmix_argv_free(alloc);
            free(nodelist);
    ...
    if (pmix_list_is_empty(&nds)) {
        /* if we get here, then we were able to contact timex,
         * which means we are in an actively managed cluster.
         * However, timex indicated that nothing is currently
...
/* we cannot use the RML to communicate with timex as it doesn't
...
static int dyn_allocate(prte_job_t *jdata)
{
    ...
    int i;
    struct timeval tv;
    local_jobtracker_timex_t *jtrk;
    int64_t i64, *i64ptr;

    if (NULL == prte_ras_timex_component.config_file) {
        prte_output(0, "Cannot perform dynamic allocation as no timex configuration file provided");
        return PRTE_ERR_NOT_FOUND;
    }

    /* track this request */
    jtrk = PMIX_NEW(local_jobtracker_timex_t);
     ...
     */
#if 0
    if (!prte_ras_timex_component.rolling_alloc) {
    ...
    /* pass the timeout */
    pmix_asprintf(&tmp, "timeout=%d", prte_ras_timex_component.timeout);
    pmix_argv_append_nosize(&cmd, tmp);
    free(tmp);
     ...
     * in the defined time, then we will error out as timex isn't
     * responding to us
     */
    prte_event_evtimer_set(prte_event_base, &jtrk->timeout_ev, timeout, jtrk);
    tv.tv_sec = prte_ras_timex_component.timeout * 2;
    tv.tv_usec = 0;
    prte_event_evtimer_add(&jtrk->timeout_ev, &tv);

    prte_output_verbose(2, prte_ras_base_framework.framework_output,
                        "%s timex:dynalloc cmd_str = %s", PRTE_NAME_PRINT(PRTE_PROC_MY_NAME),
                        cmd_str);
...
static int parse_alloc_msg(char *msg, int *idx, int *sjob, char **nodelist, char **tpn)
{
    ...
    p_str = strtok(tmp, " ");
    while (p_str) {
        if (NULL != strstr(p_str, "timex_jobid")) {
            pos = strchr(p_str, '=');
...
{
    FILE *fp;
    char line[PRTE_timex_DYN_MAX_SIZE];
    char *pos;
    bool found_port = false;
    bool found_ip = false;

    if (NULL == (fp = fopen(filename, "r"))) {
        pmix_show_help("help-ras-timex.txt", "config-file-not-found", true, filename);
        return PRTE_ERR_SILENT;
    }

    memset(line, 0, PRTE_timex_DYN_MAX_SIZE);
    while (NULL != fgets(line, PRTE_timex_DYN_MAX_SIZE, fp) && (!found_ip || !found_port)) {
        if (0 == strlen(line)) {
            continue;
        }
    ...
    fclose(fp);
    if (!found_ip) {
        prte_output(0, "The IP address or name of the timex control machine was not provided");
        return PRTE_ERR_NOT_FOUND;
    }
    if (!found_port) {
        prte_output(0, "The IP port of the timex dynamic allocation service was not provided");
        return PRTE_ERR_NOT_FOUND;
    }
